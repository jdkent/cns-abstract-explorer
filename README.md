...
If `.env` contains `OPENAI_API_KEY`, the full pipeline also asks OpenAI to name each non-noise cluster from the titles in that cluster. If the first pass produces duplicate labels, the pipeline submits the conflicting title sets again and asks for distinct names while preserving the cluster-id mapping. Those names are written into the `cluster_name` column in the clustering CSV and reused by the HTML-only rebuild, so cached rebuilds do not make extra API calls. You can optionally set `OPENAI_CLUSTER_LABEL_MODEL` in `.env`; otherwise the pipeline uses `gpt-4o-mini`.
